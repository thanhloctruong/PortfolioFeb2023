      ...
      */
      ...
      pImg.functions.image.init = function () {
        ...
        pImg.image.obj.addEventListener("load", function () {
          ...
          pImg.image.aspect_ratio =
            pImg.image.obj.width / pImg.image.obj.height;
          pImg.functions.image.resize();
      ...
      ========================================
      */
      pImg.functions.particles.SingleImageParticle = function (
        init_xy,
        dest_xy
      ) {
        this.x = init_xy.x;
      ...
      pImg.functions.particles.SingleImageParticle.prototype.draw =
        function () {
          pImg.canvas.context.fillStyle = this.color;
          // console.log(this.color);
          pImg.canvas.context.beginPath();
          pImg.canvas.context.arc(
            this.x,
            this.y,
            this.radius,
            0,
      ...
      pImg.functions.particles.createImageParticles = function (
        ...
        const increment = Math.round(
          pixel_data.width / pImg.particles.density
        );
          ...
          for (let j = 0; j < pixel_data.height; j += increment) {
            if (pixel_data.data[(i + j * pixel_data.width) * 4 + 3] > 128) {
              ...
              pImg.particles.array.push(
                new pImg.functions.particles.SingleImageParticle(
                  init_xy,
                  dest_xy
                )
              );
      ...
      */
      pImg.functions.interactivity.repulseParticle = function (p, args) {
        ...
        const dx_mouse = p.x - pImg.mouse.x,
          ...
          inv_strength = pImg.functions.utils.clamp(
            300 - args.strength,
            10,
            300
          );
      ...
      pImg.functions.interactivity.grabParticle = function (p, args) {
        ...
        if (mouse_dist <= args.distance) {
          ...
          pImg.canvas.context.lineWidth = Math.min(
            args.line_width,
            p.radius * 2
          );
          pImg.canvas.context.beginPath();
      ...
      */
      ...
      pImg.functions.utils.addEventActions = function (event) {
        ...
        if (pImg.particles.interactivity[event].enabled) {
          const func =
              action_funcs[pImg.particles.interactivity[event].action],
            args =
              pImg.interactions[pImg.particles.interactivity[event].action];
    ...
    */
    ...
    window.particleImageDisplay = function (tag_id) {
      ...
      const pImage_el = document.getElementById(tag_id),
        ...
        existing_canvases =
          pImage_el.getElementsByClassName(canvas_classname);
